...
def test_iceberg_partitioned_write_uses_every_task_without_losing_rows(spark, sql_catalog):
    ...
    The writer asks its input to lead with the partition keys so one partition writer is live
    at a time. That sort is the regression risk here: it reorders every row before the write,
